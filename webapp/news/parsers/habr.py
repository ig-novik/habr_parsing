...
def get_news_snippets():
    print("Вход в get_news_snippets()")
    ...
    if html:
        ...
        for news in all_news:
            ...
            print(url)
            if url.find("habr.com") != -1:
                published = news.find('span', class_='post__time').text
                published = parse_habr_date(published)
                news_id = save_news(title, url, published)


def get_news_content():
    print("Вход в get_news_content()")
    news_without_text = News.query.filter(News.text.is_(None)).all()
    for news in news_without_text:
        html = get_html(news.url)
        if html:
            soup = BeautifulSoup(html, 'html.parser')
            article = soup.find('div', class_='post__text-html')
            if article:
                news_text = article.decode_contents()
                if news_text:
                    news.text = news_text
                    db.session.add(news)
                    db.session.commit()
